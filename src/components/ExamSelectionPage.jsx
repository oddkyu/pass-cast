  ...
  return (
    ...
    >
      <header className={`sticky top-0 z-50 border-b transition-all duration-500 ${isDarkMode ? 'bg-midnight/40 border-white/5 backdrop-blur-2xl' : 'bg-white/80 border-slate-100 backdrop-blur-md'}`}>
        <div className="max-w-7xl mx-auto px-6 md:px-16 h-20 md:h-28 flex items-center space-x-6 md:space-x-8">
          <button onClick={handleBack} className="w-10 h-10 md:w-12 md:h-12 glass-button rounded-xl md:rounded-2xl flex items-center justify-center">
             <svg width="20" height="20" md:width="24" md:height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M15 18l-6-6 6-6"/></svg>
          </button>
          <div className="flex flex-col">
            <span className="text-[10px] md:text-[12px] font-black text-gold uppercase tracking-[0.3em] mb-1">
...
const YearCard = ({ year, round, isDarkMode, onClick, index }) => (
  <motion.button
    ...
    onClick={onClick}
    className={`group flex items-center p-6 md:p-10 rounded-[2.5rem] md:rounded-[3rem] text-left relative overflow-hidden transition-all duration-500
  ...
  >
    ...
    <div className={`w-14 h-14 md:w-24 md:h-24 rounded-xl md:rounded-[2rem] flex flex-col items-center justify-center mr-4 md:mr-8 flex-shrink-0 transition-all duration-500
      ${isDarkMode ? 'bg-gold/10 text-gold border border-gold/20' : 'bg-midnight text-gold shadow-xl'}
    `}>
      <span className="text-[9px] md:text-[12px] font-black mb-0.5">{year}</span>
      <span className="text-base md:text-2xl font-black tracking-tighter leading-none">{round}</span>
    </div>
    <div className="flex-1 min-w-0">
      <h3 className={`text-lg md:text-2xl font-black mb-1 whitespace-nowrap overflow-hidden text-ellipsis ${isDarkMode ? 'text-white' : 'text-midnight'}`}>공인중개사 기출</h3>
      <p className={`text-[11px] md:text-sm font-bold whitespace-nowrap overflow-hidden text-ellipsis ${isDarkMode ? 'text-white/30' : 'text-slate-400'}`}>전체 과목 실전 풀기</p>
    </div>
    <div className={`w-10 h-10 md:w-12 md:h-12 rounded-xl md:rounded-2xl flex items-center justify-center transition-all duration-500 flex-shrink-0
...
const SubjectCard = ({ subject, isDarkMode, onClick, index }) => (
  ...
  >
    <div className="flex flex-col space-y-4">
      ...
      <div>
        <h3 className={`text-lg md:text-[20px] font-black leading-tight mb-1 break-keep ${isDarkMode ? 'text-white' : 'text-midnight'}`}>{subject.name}</h3>
        <p className={`text-[10px] md:text-xs font-bold ${isDarkMode ? 'text-white/30' : 'text-slate-400'}`}>{subject.info}</p>
      </div>
    </div>
  </motion.button>
);
